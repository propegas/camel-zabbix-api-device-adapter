...
public class ZabbixAPIConsumer extends ScheduledPollConsumer {
	...
	private int processSearchDevices() throws ClientProtocolException, IOException, Exception  {
		...
		try {
			...
			List<Device> devicehosts = getAllHosts(zabbixApi);
	...
	private List<Device> getAllHosts(DefaultZabbixApi zabbixApi) {
		...
		try {
	       ...
	       // filter.put("host", new String[] { host1, host2 });
	        ...
	        getRequest = RequestBuilder.newBuilder()
	                ...
	                .paramEntry("selectItems", new String[] { "itemid", "name", "key_" })
	                .paramEntry("output", new String[] { "hostid", "name", "host" })
	                .paramEntry("selectMacros", new String[] { "hostmacroid", "macro" })
	                .paramEntry("selectGroups", "extend")
	                .paramEntry("selectParentTemplates", new String[] { "templateeid", "name" })
	                .build();
		...
		for (int i=0; i<hosts.size(); i++) {
			...
			JSONArray hostgtemplates = host.getJSONArray("parentTemplates");
			...
			for (int j=0; j<hostgroups.size(); j++) {
				...
				logger.info("**** Received JSON Hostgroup: " + hostgroup.toString() );
			}
			
			for (int x=0; x<hostgtemplates.size(); x++) {
				//logger.debug(f.toString());
				//ZabbixAPIHost host = new ZabbixAPIHost();
				JSONObject hostgtemplate = hostgtemplates.getJSONObject(x);
				//JSONArray hostgroups = host.getJSONArray("groups");
				
				logger.info("**** Received JSON Hosttemplate: " + hostgtemplate.toString() );
			}
			
			//Device gendevice = new Device();
			//gendevice = genHostObj( host, "host" );
			...
			//logger.info("Finded fcSwitches: "+ fcSwitches.size() + " for FCfabric " + fckey);
	...
	/*
		...
		for (JsonElement f : hosts) {
			logger.debug(f.toString());
			ZabbixAPIHost host = new ZabbixAPIHost();
			host = gson.fromJson(f, ZabbixAPIHost.class);
	...
	private JsonObject performPostRequest(String method, JsonElement jsonparams, String auth) throws ClientProtocolException, IOException {
		...
		*/	
		...
		//request.setEntity(new StringEntity(new UrlEncodedFormEntity(payload), "UTF8"));
	...
	private Device genHostObj( ZabbixAPIHost host, String device_type ) {
...
<xs:enumeration value="OFFLINE"/>
		...
		gendevice.setName(host.getName());
		gendevice.setId(host.getHostid());
	...
	private CloseableHttpClient HTTPinit(RequestConfig globalConfig, CookieStore cookieStore){
	    ...
	    //logger.debug("*** Received cookies: " + context.getCookieStore().getCookies());
